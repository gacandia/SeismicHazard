...
if haz.sbh(3)==1
    ...
    m8        = strcmp(mechs,'shallowcrustal');
    lambda1   = [...
        nansum(lambda1(m1,:),1);
        nansum(lambda1(m2,:),1);
        nansum(lambda1(m3,:),1);
...
%% -------------  plot hazard--------------------------------------
y0(y0<0)=nan;
plot(handles.ax2,im',y0','-','ButtonDownFcn',{@myfun,handles},'tag','lambda0','linewidth',2);
...
if haz.sbh(3)==1
    mechs = {'system','interface','intraslab','slab','crustal','fault','grid','shallowcrustal'};
