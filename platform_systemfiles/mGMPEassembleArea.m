...
for jj=1:Ndepend
    ...
    if strcmpi(gmpe.type,'cond')
        ...
        saunits = gmpe.cond.saunits;
        param   = [{M,rrup,func,usp.mechanism,saunits,source.Vs30},param]; %#ok<AGROW>
    end
    ...
    if isfrn
        ...
        switch gmpe.usp.(GMMjj).type
            case 'regular'
                PARAM {jj}=param;
            case 'cond'
                saunits   = gmpe.usp.(GMMjj).cond.saunits;
                PARAM {jj}= [{M,rrup,gmpe.usp.(GMMjj).cond.conditioning,usp.mechanism,saunits,source.Vs30},param]; %#ok<AGROW>
            case 'udm'
                PARAM {jj}=param;
            case 'pce'
                PARAM {jj}=param;
