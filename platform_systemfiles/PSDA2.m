...
handles.runREG.CData = double(imread('Play.jpg'))/255;
...
handles.runCDM.CData = double(imread('PlayCDM.jpg'))/255;
handles.CDM_DisplayOptions.CData = double(imread('Settings.jpg'))/255;
...
handles.REG_Display   = dispoptions1;
handles.CDM_Display   = dispoptions2;
handles.haz           = [];
handles.haz2          = [];

% coloroptions
handles.col={[0 0.447 0.741],2,1,[0.76 0.76 0.76],'gray',0.5};
...
function setColors_Callback(hObject, eventdata, handles)

handles.col=ColorManager(handles.col);
switch handles.mode1.Value
    case 1, plot_PSDA_regular(handles)
    case 0, plot_PSDA_cdm(handles)
end
guidata(hObject,handles)
